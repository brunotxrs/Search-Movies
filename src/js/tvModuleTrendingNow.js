import {  jsonDataTv, genrePromiseTv, fetchTvCertification, fetchTvByGenre  } from './fetch_api.js';
...
export async function trendingNowTv(filterType = 'all'){
    ...
    try {
        if (filterType === 'all') {
            const tvData = await jsonDataTv;
            tvToDisplay = tvData.results;
            ...
            if (selectedGenreId) {
                const genreTvData = await fetchTvByGenre(selectedGenreId);
                tvToDisplay = genreTvData.results;
        ...
        if (tvToDisplay.length === 0) {
            cardsHtml.innerHTML = '<p>Nenhum filme encontrado para esta seleção.</p>';
            return;
        }


        const classificationPromises = tvToDisplay.map(tv => 
            fetchTvCertification(tv.id, 'BR')
        );

        const allClassifications = await Promise.all(classificationPromises);

        tvToDisplay.forEach((element, index) => {
            const imagePath = element.poster_path;
            const imageUrl = `${TMDB_IMAGE_BASE_URL}${imagePath}`;
            
            let tvGenreName = (filterType === 'all' && element.genre_ids && element.genre_ids.length > 0)
                            ? (genreMap.get(element.genre_ids[0]) || 'Desconhecido')
                            : filterType;

            const tvClassification = allClassifications[index];
            const classificationText = tvClassification ? `${tvClassification}+` : '+'; // Se vazio, coloca só '+'

            const details = `
                <span class="card_left_right" id="card_id_${index}">
                    <img src="${imageUrl}" alt="${element.name}" class="img">

                    <div class="area_of_types ${states.class_hidden}">
                        <span class="classifications">${classificationText}</span> 
                        <span class="movie-genre">${tvGenreName}</span> 
                    ...
                    <span class="name_filme ${states.class_hidden}" id="name_filme_${index}">${element.name}</span>
                </span>
            `;
        ...
        const imgTrending = document.querySelectorAll('.img')
        imgTrending.forEach((e, i )=> {
            
            if(e.src === `${TMDB_IMAGE_BASE_URL}null`){
                e.classList.add(states.class_hidden)
            }
    ...
    function applyCardHoverEffects() {
        const cardsTv = document.querySelectorAll('.card_left_right');
        cardsTv.forEach((cardElement) => {
